...
// Valore sentinella per l'opzione "Altro": nessun carrello reale ha questo codice.
const CODICE_ALTRO = "ALTRO";
...
export default function RegistrazioneForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Il <select> ragiona per codice: qui si risale alla descrizione leggibile
    // (usata nel certificato e nelle email) e si tiene comunque il codice.
    const codiceModelloScelto = (formData.get("codiceModello") as string) || "";
    if (!codiceModelloScelto) {
      setError("Seleziona il modello del carrello.");
      setLoading(false);
      return;
    }
    let modello: string;
    let codiceModello: string | null;
    if (codiceModelloScelto === CODICE_ALTRO) {
      modello = "Altro";
      codiceModello = null;
    } else {
      const trovato = carrelli.find((c) => c.codice === codiceModelloScelto);
      modello = trovato?.descrizione || codiceModelloScelto;
      codiceModello = codiceModelloScelto;
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <select name="codiceModello" required defaultValue={precompilazione?.codiceModello || ""} className="w-full border border-gray-300 rounded-lg px-3.5 py-2.5 focus:ring-2 focus:ring-[#1A4731] focus:border-transparent outline-none transition bg-white">
                ...
                <option value={CODICE_ALTRO}>Altro</option>
              </select>
            </div>
